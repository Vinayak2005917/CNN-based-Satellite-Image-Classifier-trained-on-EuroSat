...
for idx in range(len(test_df)):
    img_rel_path = test_df.iloc[idx, 1]       # relative image file path
    class_name = test_df.iloc[idx, 3]         # class name column (string)
    
    img_path = os.path.join(path, "EuroSAT", img_rel_path)
    img = Image.open(img_path).convert("RGB")

    # add class name at the start of filename
    fname = os.path.basename(img_rel_path)
    new_filename = f"{class_name}_{fname}"
